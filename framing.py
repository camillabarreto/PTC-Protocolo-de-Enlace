...
class Framing(Sublayer):
    ...
    def handle(self):
        ...
        if (result != None): # Verifica se leu todos os bytes
    ...
    def send(self, msg):
        ...
        self.fcs = crc.CRC16(msg)
        msg_crc = self.fcs.gen_crc()
    ...
    def read(self, byte):
        if (byte[0] == FLAG):
            self.fcs.clear()
            ''' Testando a verificação com CRC '''
            # self.msg = self.msg[:-1] # Um ou mais octetos corrompidos
            # self.msg.append(2) # Octetos adicionais no quadro
            self.fcs.update(self.msg)
            if(self.fcs.check_crc()): # Verifica se a mensagem está corrompida
                self.current_state = IDLE
                print('read -> idle')
                self.disable_timeout()
                return self.msg
